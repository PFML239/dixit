...
#conn1.execute("CREATE INDEX speedd ON CORRELATIONS(IDCRE,IDCHAR)")
"""
...
if (len(s2) == 2):
    ...
    if (chosen2 > - 1): 
        ...
        n = n + 1
        #l1.append(s3)
        conn1.execute("INSERT INTO ANIMALS (NAME) VALUES (:s3)",{"s3": s3})
        for i in range(0,m):
            conn1.execute("INSERT INTO CORRELATIONS (IDCRE, IDCHAR, IND) VALUES (:a,:b,:c)",{"a": n - 1, "b": i, "c": 0})
        for i in range(0,vop):
        ...
        if (bo == 0):
            #l2.append(s4)
            m = m + 1
            conn1.execute("INSERT INTO CHARACTERISTICS (NAME) VALUES (:s4)",{"s4": s4})
            for i in range(0,n):
                conn1.execute("INSERT INTO CORRELATIONS (IDCRE, IDCHAR, IND) VALUES (:a,:b,:c)",{"a": i, "b": m - 1, "c": 0})
            ...
            print("Обладает ли ",s3," добавленным Вами признаком?")
            s6 = input()
            if (len(s6) == 2):
                #a[n - 1][m] = 1
                conn1.execute("UPDATE CORRELATIONS SET IND = :w WHERE IDCRE = :u AND IDCHAR= :v",{"w": 1,"u": n - 1,"v": m - 1})
            elif (len(s6) == 3):
                #a[n - 1][m] = -1
                conn1.execute("UPDATE CORRELATIONS SET IND = :w WHERE IDCRE = :u AND IDCHAR= :v",{"w": -1,"u": n - 1,"v": m - 1})
            else:
                #a[n - 1][m] = 0
                conn1.execute("UPDATE CORRELATIONS SET IND = :w WHERE IDCRE = :u AND IDCHAR= :v",{"w": 0,"u": n - 1,"v": m - 1})
            ...
            for y in range(0,que):
                ...
                if (len(s5) == 2):
                    #a[indd][m] = 1
                    conn1.execute("UPDATE CORRELATIONS SET IND = :w WHERE IDCRE = :u AND IDCHAR= :v",{"w": 1,"u": indd,"v": m - 1})
                elif (len(s5) == 3):
                    #a[indd][m] = -1
                    conn1.execute("UPDATE CORRELATIONS SET IND = :w WHERE IDCRE = :u AND IDCHAR= :v",{"w": -1,"u": indd,"v": m - 1})
                else:
                    #a[indd][m] = 0
                    conn1.execute("UPDATE CORRELATIONS SET IND = :w WHERE IDCRE = :u AND IDCHAR= :v",{"w": 0,"u": indd,"v": m - 1})
conn1.commit();
conn1.close();
